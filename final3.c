...
[Acknowledgements: Jonathan Leffler's padding method from
https://stackoverflow.com/questions/15312482/how-to-center-text-in-c-console
*/
...
/* This function clears the terminal screen
    Precondtions: Assumes a UNIX-Like/MacOS or Windows terminal. Uses the macros
   __unix__ and _WIN32 to determine which OS is being used.
...
/* This function loads dummy data into the array
    Precondtions: Assumes that ID and PIN is an integer and balance is a double
   for elements {x,y,z} respectively and that the array is of size 10
...
void load_data(struct account *accounts) {
  struct account users[] = {{1800, 234567, 1000}, {6765, 123456, 1000},
                            {2584, 111111, 1000}, {1009, 222222, 1000},
                            {5555, 333333, 1000}, {4444, 444444, 1000},
                            {3333, 555555, 1000}, {2222, 666666, 1000},
                            {1111, 777777, 1000}, {8203, 888888, 1000}};
...
/* This function prints the accounts for debugging
    Precondtions: assumes a size given by the macro SIZE. Assumes that ID and
   PIN is an integer and balance is a double for elements {x,y,z} respectively
   and that each element of the array is a struct account.
    @param struct account *accounts (pointer to the struct array of accounts)
...
are valid
    Precondtions: assumes a size by the macro SIZE. Assumes that ID and PIN is
an integer and balance is a double for elements {x,y,z} respectively and that
each element of the array is a struct account. Preconditions 2: Also assumes
that both id and pin are integers.
    @param struct account *accounts (pointer to the struct array of accounts)
...
/* This function gives a login form to the user
    Precondtions: assumes a size by the macro SIZE. Assumes that ID and PIN is
   an integer and balance is a double for elements {x,y,z} respectively and that
   each element of the array is a struct account.
...
/* This function determines whether the user reached the maximum number of tries
   ...
   an integer and balance is a double for elements {x,y,z} respectively and that
   each element of the array is a struct account. Preconditions 2: Also assumes
   that atm status is a "boolean" integer.
...
/* This function is a withdrawal function used by cluster 1
    Precondtions: assumes a size of 10, assumes every element in struct is an
   integer except the balance which is a positive double which is a positive
   double which is a positive double which is a positive double.
    @param struct account *accounts
...
void withdrawMoney(struct account *accounts, int index, double *atm_balance,
                   double amount) {
  if (amount > 0) {
...
/* This function is a foreign currency exchange function used by cluster 3
    Precondtions: assumes a size of 10, assumes every element in struct is an
   positive integer except the balance which is a positive double which is a
   positive double which is a positive double which is a positive double.
    @param struct account *accounts
    @param int index (index of the account in accounts)
    @param int *atm_balance (atm source)
    @param int amount (amount to withdraw)
    ...
    if (exchange_rate == 0) {
      ...
      if (amount > accounts[index].balance || amount > *atm_balance) {
        ...
        *atm_balance -= amount / (1 + tax);
        printf(
            "You have successfully converted %lf to the specified currency.\n",
            amount);
      }
    }
  } else {
...
/* This function is a function which checks if an integer is part of the
   fibonacci sequence Precondtions: assumes input is an integer
    @param int id (number to check)
    @return true or false (boolean)
*/
...
/* This function is a function which checks if an integer is divisible by 3 and
   5 Precondtions: assumes input is an integer
    @param int id (number to check)
    @return true or false (boolean)
*/
...
/* This function checks the available billers for a certain id by iterating
   through the various conditions then adding them to a string array.
    Preconditions: assumes a size of 10, assumes every element in struct is an
   integer except the balance which is a positive double which is a positive
   double which is a positive double which is a positive double which is a
   positive double. assumes that the index is a pointer to a valid integer. and
   that the currentBillers is a pointer to a string array. Precondtions: assumes
   input is an integer
    @param struct account *accounts (pointer to the array of accounts)
    @param int index (index of the account in accounts)
...
void getAvailableBillers(struct account *accounts, int *index,
                         char *currentBillers[]) {
  int i = 0;
  if (1) {
...
/* A function to print available payments from a string array of billers also
   uses paddding to center text. Uses a ternary operator to check if the length
   of the string is greater than the specified width. If it is, it will print
   the string as is. If it is not, it will print the string with padding.
    @credits: Adaptation of Jonathan Leffler's padding method from
   https://stackoverflow.com/questions/15312482/how-to-center-text-in-c-console
    Precondtions: assumes input is a string array. Assumes that there is a
   single NULL element at the end of the array. The array must be at least 1
   element long.
    @param char *currentBillers (pointer to the array of billers)
*/
...
/* A generic bill function as a placeholder for the bill functions
    Precondtions: assumes that the struct account *accounts is not null and set
   by the macro SIZE. also assumes that the function args has the same type as
...
/* This function checks the value of the current option or biller or action. The
   function genericBill is used as a placeholder for the bill functions.
   Improves modularity and lessens code duplication. Precondtions: assumes that
   the struct account *accounts is not null and set by the macro SIZE. also
   assumes that the function args has the same type as the specified parameters
   .
...
int checkAction(struct account *accounts, int *index, char *currentBillers[],
                int choice) {
...
/* This function serves as the menu for the payBill function.
    Precondtions: assumes struct account *accounts is set by the macro SIZE,
   also assumes that the specified types within the function args are the same
   as user input.
    @param struct account *accounts (pointer to the array of accounts)
    @param int index (index of the account in accounts)
*/
...
void payBills(struct account *accounts, int *index,
              double *available_withdrawal, double *available_forex) {
  clearTerminal();
  printf("---------------------------------------------------------------------"
         "------");
  printf("\n  Bank X Digital Banking System - Remaining Balance: %.0lf PHP, "
         "%.0lf ForEx\n",
...
/* This function gives the user various options to choose from for shopping
   online. Might use various APIs in the future to connect to online stores such
   as Lazada, Shopee, etc. Precondtions: assumes a size of 10, assumes every
   element in struct is an integer except the balance which is a positive double
   which is a positive double which is a positive double which is a positive
   double which is a positive double.
    @param struct account *accounts
    @param int index (index of the account in accounts)
    @return none
*/

void onlineShop(struct account *accounts, int *index) { printf("Error!"); };

void loggedIn(struct account *accounts, int *index,
              double *available_withdrawal, double *available_forex,
              int isDebug) {
  clearTerminal();
